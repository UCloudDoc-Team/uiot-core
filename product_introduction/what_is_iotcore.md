...
## 客户收益（这里需要修改）
- 提供完整的设备端SDK，方便用户以极低的研发成本及周期快速接入云端，标准化设备注册认证流程。
- 提供设备影子、物模型、规则引擎功能、OTA，实现设备到云端、设备到设备、云端到设备的数据交互。用户只需利用这些功能，专注于业务侧开发，无需关注底层基础功能。
- 平台提供了海量设备并发连接与高性能消息吞吐的能力，用户可稳定获取海量设备数据，方便用户获取终端设备/终端用户的运行数据或行为习惯，支撑后续产品开发/产品管理/生产管理的判断依据。
