...
- Topic用于规则引擎筛选时，支持MQTT默认的通配符`+`，`#`，参考[Topic通配符](uiot-core/console_guide/product_device/topic#Topic通配符)；
...
## 系统Topic
系统Topic是指系统预定义的有特殊用途的Topic，比如操作[设备影子](uiot-core/console_guide/device_shadow/operation_guide)、操作[物模型](uiot-core/console_guide/thingmode/what_is_thingmode)、操作设备状态、操作[固件升级](uiot-core/console_guide/ota/what_is_ota)，具体概念可以参考对应章节。
...
7. 自定义Topic添加完成后
   - 修改：点击<修改>，修改Topic内容；
   - 删除：点击<删除>，删除该Topic，参考[Topic删除](uiot-core/console_guide/product_device/topic#Topic删除)；
